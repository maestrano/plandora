	...
	public ActionForward showChart(ActionMapping mapping, ActionForm form,
	    ...
	    try {
		    ...
		    if (newChartId!=null && !newChartId.equals("")) {
			    ...
			    if (gad!=null) {
			    	ReportTO rto = rdel.getReport(new ReportTO(newChartId));
				    UserTO uto = SessionUtil.getCurrentUser(request);
				    PreferenceTO pref = uto.getPreference();
					pref.addPreferences(new PreferenceTO(gad.getId() + ".KPI_ID", newChartId, uto));
					if (rto!=null && rto.getProject()!=null && !rto.getProject().getId().equals("0") && !rto.getProject().getId().equals("")) {
						pref.addPreferences(new PreferenceTO(gad.getId() + ".KPI_PROJECT", rto.getProject().getId(), uto));	
					} else {
						pref.addPreferences(new PreferenceTO(gad.getId() + ".KPI_PROJECT", frm.getProjectId(), uto));
					}
