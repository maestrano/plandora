...
public class PTableHelper {
		...
		if (table!=null && uto!=null && datagrid!=null){			
	        ...
	        if (table.getColumnsNumber() > 0) {
	            for (int i = 0; i < table.getRealColumnsNumber(); i++) {
	                ...
	                //col = col + column.getColumnsNumber(req, name);
	            }        	
	        } else {
        ...
        if (req!=null && req.getSession()!=null) {
    		...
    		if (table!=null && uto!=null && datagrid!=null){
    	        ...
    	        if (datagrid.size()>0) {
    		        ...
    		        int pageSize = this.getPageSize(table, uto);
	        	...
	        	if (column.containComboFilter()) {
	        		...
	        		if (filterValue!=null) {
		        		for (String content : values) {
		    				if (!filterValue.trim().equals("") && !filterValue.equals(content)) {
		    					try {					
	...
	private StringBuffer getFilterComboBox(Collection datagrid, PTableTag table, UserTO uto) {
			...
			for (PColumnTag column : columns) {
				...
				if (column.containComboFilter()) {
					...
					if (list==null) {
						...
						hm.put("" + colNum, list);
						filterIds.add("" + colNum);
						colTitles.add(column.getTitle());
					}
		...
		for (int i=0; i<filterIds.size(); i++) {
			String cmbId = "filter_" + table.getName() + "_" + filterIds.get(i);
			HashMap<String, String> list = hm.get(filterIds.get(i));
			response.append("<select id=\"" + cmbId + "\" name=\"" + colTitles.get(i) + "\"  class=\"textBoxSmall\" " +
					"onchange=\"javascript:requestPTableBody('" + table.getName() + "', '" + table.getFrm() + "', '" + cmbId + "=' + this.value);return false;\">");
			response.append("<option value=\"\">" + uto.getBundle().getMessage(uto.getLocale(), colTitles.get(i)) + "...</option>");
			
			String currentVal = table.getParameter(cmbId);
			for (String item : list.values()) {
				String selectedLabel = "";
				if (currentVal!=null && currentVal.equals(item)) {
					...
					if (currentVal!=null) {
						try {
							...
							if (currentValWithEncoding!=null && currentValWithEncoding.equals(item)) {
								selectedLabel = "selected";		
							}
						} catch (UnsupportedEncodingException e) {
							e.printStackTrace();
				...
				response.append("<option " + selectedLabel + " value=\"" + item + "\">" + item + "</option>");	
			}
	...
	private int getPageSize(PTableTag table, UserTO uto) {
		int response = 0;
