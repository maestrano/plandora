...
public final class PredictableTaskChartGadget extends ChartGadget {

	private static final String PREDIC_TASK_PROJECT = "PROJECT";
	
	private static final String PREDIC_TASK_INTEVAL = "INTERVAL";
	
	private HashMap granularity = new HashMap();
	

	public String getUniqueName(){
		return "label.manageOption.gadget.predicTask";
	}
	
	public String getId(){
		return "PREDIC_TASK_CHART";
	}

	@Override	
    public Vector<TransferObject> getFieldsId(){
    	Vector<TransferObject> response = new Vector<TransferObject>();
       	response.add(new TransferObject(PREDIC_TASK_PROJECT, "-1"));
       	response.add(new TransferObject(PREDIC_TASK_INTEVAL, "1"));
        return response;
    }
    ...
    public Vector getFields(){
    	...
    	try {
        	...
        	response.add(new FieldValueTO(PREDIC_TASK_PROJECT, "label.manageOption.gadget.predicTask.project", projList));
        	
            Vector intervList = new Vector();
            intervList.add(new TransferObject("1", "label.manageOption.gadget.predicTask.interval.1"));
            intervList.add(new TransferObject("2", "label.manageOption.gadget.predicTask.interval.2"));
            intervList.add(new TransferObject("3", "label.manageOption.gadget.predicTask.interval.3"));
            intervList.add(new TransferObject("4", "label.manageOption.gadget.predicTask.interval.4"));
        	response.add(new FieldValueTO(PREDIC_TASK_INTEVAL, "label.manageOption.gadget.predicTask.interval", intervList));
	...
	public String generate(Vector selectedFields)  throws BusinessException {
        ...
        try {
        	String intervType = super.getSelected(PREDIC_TASK_INTEVAL, selectedFields);
        	ProjectTO pto = pdel.getProjectObject(new ProjectTO(super.getSelected(PREDIC_TASK_PROJECT, selectedFields)), true);
            ...
            if (pto!=null) {
                ...
                String sql = "select sub.is_unpredictable, sub.bucket_date, sub.alloc_time from (" +
                		"select t.is_unpredictable, a.alloc_time, ";
                ...
                String defectLbl = super.getI18nMsg("label.manageOption.gadget.predicTask.label.1");
                String nonDefectLbl = super.getI18nMsg("label.manageOption.gadget.predicTask.label.2");
                ...
                response = "{ \n" + 
                	getJSonTitle() + "," +
                	getJSonYLegend("(h)") + "," + 
                	getBarStackValues(dataVals, labels) + "," +
                	getJSonAxis(xaxis, null, "x_axis") + "," + getJSonAxis(null, dataVals, "y_axis") + "}";
            } else {
