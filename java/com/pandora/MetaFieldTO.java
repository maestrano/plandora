...
public class MetaFieldTO extends TransferObject {
    ...
    public StringBuffer getFormatedField(String style, String currValue, UserTO uto){  
    	return getFormatedField(style, currValue, uto, false);
    }
    
    /**
     * Format a html field with title and body.
     */
    public StringBuffer getFormatedField(String style, String currValue, UserTO uto, boolean isReadOnly){    	
        StringBuffer buff = new StringBuffer();
        ...
        if (this.getType().equals(new Integer(TYPE_TEXT_BOX))) {
            ...
            buff.append(HtmlUtil.getComboBox(this.getHtmlName(), this.getDomain(), style, currValue, null, 0, null, isReadOnly) );
           	buff.append(getHelpNote());	
		...
		for (int j = 0; j < cols.getLength(); j++) {
			...
			if (colNode != null) {
				...
				if (colType != null) {
					...
					payload.append(innerField.getFormatedField(style, currentValueOfCell, uto, isReadOnly));
					counter++;
