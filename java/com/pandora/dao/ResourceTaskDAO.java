...
public class ResourceTaskDAO extends DataAccess {
	...
	private void getRemainingFields(ResultSet rs, ResourceTaskTO rrto) throws DataAccessException, BusinessException {
		...
		String req = getString(rs, "requeriment_id");
    ...
    private String getTaskListByResourceQuery(){
        ...
        response = "select rt.estimated_time, rt.start_date, rt.task_status_id, rt.resource_id, rt.billable, " +
					    ...
					    "po.value, pr.name as PROJECT_NAME, u.username, t.requeriment_id, t.created_by, t.category_id," +
					...
					"from resource_task rt, tool_user u, " +
						 "task t LEFT OUTER JOIN (select id, iteration, description as REQ_DESC from planning) as req on req.id = t.requeriment_id, " +
    ...
    private Vector<ResourceTaskTO> getListByProject(ProjectTO pto, String statusSel, String resourceSel, String dateRangeSel, Connection c) throws DataAccessException {
		...
		try {
			...
			String sql = "select * from (" +
					   ...
					   "t.name as task_name, t.category_id, c.name as CATEGORY_NAME, c.billable as CATEGORY_BILLABLE, t.requeriment_id, " +
					   ...
					   "LEFT OUTER JOIN (select id, iteration, description as REQ_DESC from planning ) as rp on rp.id = t.requeriment_id " +
					   "where u.id = rt.resource_id " +
		    ...
		    sql+=" group by rt.task_id, rt.resource_id, rt.project_id, rt.billable, rt.start_date, rt.estimated_time, rt.actual_date, rt.actual_Time, rt.task_status_id, " +
		    	 "u.username, ts.state_machine_order, ts.name, p.iteration, t.name, t.category_id, c.name, c.billable, t.requeriment_id, cnt.instance_id, " +
			...
			while (rs.next()){
			    ...
			    RequirementTO req = new RequirementTO(getString(rs, "requeriment_id"));
