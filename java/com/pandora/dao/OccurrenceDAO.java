...
public class OccurrenceDAO extends PlanningDAO {
	...
	public TransferObject getObject(TransferObject to, Connection c)
		...
		try {
			...
			pstmt = c
					.prepareStatement("select o.id, o.source, o.project_id, o.name, o.occurrence_status, pr.name as PROJECT_NAME, "
							+ "o.occurrence_status_label, p.visible, o.loc, p.description, p.creation_date, p.final_date "
			...
			if (rs.next()) {
				...
				response.setAdditionalFields(afdao.getListByPlanning(response,
						null, c));
				response.setDiscussionTopics(dtdao.getListByPlanning(response,
						c));
			}
	...
	private Vector<OccurrenceFieldTO> getFieldList(OccurrenceTO oto,
			Connection c) throws DataAccessException {
		...
		try {
			...
			pstmt = c
					.prepareStatement("select occurrence_id, field, value, date_value from occurrence_field "
							+ "where occurrence_id=?");
			pstmt.setString(1, oto.getId());
			rs = pstmt.executeQuery();
			while (rs.next()) {
				OccurrenceFieldTO ofto = this
						.populateObjectByResultSet(oto, rs);
	...
	public Vector<OccurrenceTO> getIterationListByProject(String projectId,
			Connection c) throws DataAccessException {
		Vector<OccurrenceTO> response = new Vector<OccurrenceTO>();
		ResultSet rs = null;
		PreparedStatement pstmt = null;
		try {
			...
			pstmt = c
					.prepareStatement("select o.id, o.source, o.project_id, o.name, "
	...
	public void update(TransferObject to, Connection c)
			throws DataAccessException {
		PreparedStatement pstmt = null;
		try {
	...
	public void insert(TransferObject to, Connection c)
			throws DataAccessException {
		PreparedStatement pstmt = null;
		try {
	...
	public void remove(TransferObject to, Connection c)
			throws DataAccessException {
		PreparedStatement pstmt = null;
		try {
			...
			pstmt = c
					.prepareStatement("delete from occurrence_history where occurrence_id=?");
			pstmt.setString(1, oto.getId());
			pstmt.executeUpdate();
			...
			pstmt = c
					.prepareStatement("delete from plan_relation where plan_related_id=? or planning_id=?");
			pstmt.setString(1, oto.getId());
			pstmt.setString(2, oto.getId());
			pstmt.executeUpdate();

			pstmt = c
					.prepareStatement("delete from additional_field where planning_id=?");
			pstmt.setString(1, oto.getId());
			pstmt.executeUpdate();
	...
	private void updateFieldList(OccurrenceTO oto, Connection c)
			throws DataAccessException {
		PreparedStatement pstmt = null;
		try {
			...
			if (fields != null && !fields.isEmpty()) {
				...
				while (i.hasNext()) {
					...
					pstmt = c
							.prepareStatement("insert into occurrence_field (occurrence_id, field, value, date_value) values (?,?,?,?)");
					pstmt.setString(1, oto.getId());
	...
	private void removeFieldList(OccurrenceTO oto, Connection c)
			throws DataAccessException {
		PreparedStatement pstmt = null;
		try {
	...
	private OccurrenceFieldTO populateObjectByResultSet(OccurrenceTO oto,
			ResultSet rs) throws DataAccessException {
