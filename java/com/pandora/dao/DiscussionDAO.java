...
public class DiscussionDAO extends PlanningDAO {
	...
	private Vector<TeamInfoTO> getTeamInfo(UserTO uto, Timestamp iniDate, Connection c) throws DataAccessException{
		...
		try {
			...
			Vector<ProjectTO> projList = pdel.getProjectListForWork(uto, false);
			String projectList = "";
			...
			if (projectList!=null && !projectList.trim().equals("")) {
			    pstmt = c.prepareStatement(
			    		...
			    		"union " +
			    			"select h.requeriment_id, p.description, s.id, h.comment, h.user_id, h.creation_date, 'REQ', null, null, r.project_id, null, null, null, null " +
			    			"from requeriment_history h, requeriment_status s, planning p, requeriment r " +
			    			"where h.requeriment_status_id = s.id and h.creation_date >= ? " +
			    			"and s.state_machine_order in (1, 200, 201, 202) " +
			    			"and p.id = h.requeriment_id and r.id = h.requeriment_id " +
			    		"union " +
			    		...
			    		"union " +
			    			...
			    			"LEFT OUTER JOIN (select id, project_id from requeriment) as r on r.id = a.planning_id " + 
			    		...
			    		"UNION " +
			    			...
			    			"LEFT OUTER JOIN (select id, project_id from requeriment) as r on r.id = d.planning_id " + 
