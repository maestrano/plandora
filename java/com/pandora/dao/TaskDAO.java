...
public class TaskDAO extends PlanningDAO {
    ...
    public String copyReqIteration(String requerimentId, Connection c) throws DataAccessException {
		PreparedStatement pstmt = null;
		String iteration = null;
		
		try {    	
			RequirementDAO rdao = new RequirementDAO();
			RequirementTO rto = (RequirementTO) rdao.getObject(new RequirementTO(requerimentId));

			if (rto!=null && rto.getIteration()!=null && !rto.getIteration().trim().equals("")) {				
				iteration = rto.getIteration();
				pstmt = c.prepareStatement("update planning set iteration=? where id in (select id from task where requeriment_id=?) and iteration is null");
    ...
    private Vector<TaskTO> getTaskListByProject(String projectList, Timestamp iniRange, Timestamp finalRange, Connection c) throws DataAccessException {
		...
		try {
		    ...
		    String sql = "select distinct t.id, t.name, t.project_id, t.requeriment_id, t.category_id, t.task_id, t.is_parent_task, t.created_by, p.description, " +
    ...
    public Vector<TaskTO> getTaskListByRequirement(RequirementTO rto, ProjectTO pto, boolean evenClose, Connection c) throws DataAccessException {
		...
		try {
		    ...
		    if (rto!=null){
		        reqConstraint = "AND t.requeriment_id=? ";    
		    }
		    
			pstmt = c.prepareStatement("select t.id, t.name, t.project_id, t.requeriment_id, " +
    ...
    private Vector<TaskTO> getAvailableParentTaskList(RequirementTO rto, String projectId, Connection c) throws DataAccessException {
		...
		PreparedStatement pstmt = null; 
		try {
		    String sql = "select t.id, t.name, t.requeriment_id, t.project_id, " +
		    ...
		    if (rto!=null){
		        sql+= "AND (t.requeriment_id=? OR t.requeriment_id is null)";
		    }
    ...
    public TransferObject getObject(TransferObject to, Connection c)  throws DataAccessException {
		...
		try {
			...
			pstmt = c.prepareStatement("select t.id, t.name, t.project_id, t.requeriment_id, t.category_id, " +
    ...
    public void insert(TransferObject to, Connection c) throws DataAccessException {
		...
		try {
		    ...
		    //insert data of task
			pstmt = c.prepareStatement("insert into task (id, name, project_id, requeriment_id, category_id, " +
    ...
    public void update(TransferObject to, Connection c) throws DataAccessException {
		...
		try {
			...
			pstmt = c.prepareStatement("update task set name=?, project_id=?, " +
							   		   "requeriment_id=?, category_id=?, task_id=?, is_parent_task=?, is_unpredictable=? " +
							   		   "where id=?");
    ...
    public void updateByResource(TransferObject to, Connection c) throws DataAccessException {
		...
		try {
			...
			pstmt = c.prepareStatement("update task set task_id=?, requeriment_id=? where id=?");
    ...
    private void updateTaskByTemplateInstance(Integer instanceId, String newReqId, Connection c) throws DataAccessException{
		...
		try {
			...
			while (rs.next()) {
				...
				//update the same requeriment id for each element of list...				
				pstmt = c.prepareStatement("update task set requeriment_id=? where id=? ");
				if (newReqId!=null) {
			...
			//update requeriment references into custom_node_template...			
    ...
    protected TaskTO populateBeanByResultSet(ResultSet rs) throws DataAccessException{
        ...
        String reqId = getString(rs, "requeriment_id");
        if (reqId!=null){
