	...
	tinymce.create('tinymce.themes.AdvancedTheme', {
		...
		init : function(ed, url) {
			...
			t.settings = s = extend({
				...
				theme_advanced_snips : "include_img=include_img;include_sql=include_sql",
		...
		_createSnipSelect : function() {
			var c, t = this, ed = t.editor;

			c = ed.controlManager.createListBox('snipselect', {
				title : 'advanced.snipdefault',
				onselect : function(v) {
					ed.execCommand('SnipSelected', false, v);
					return false; // No auto select
				}
			});

			if (c) {				
				each(ed.getParam('theme_advanced_snips', t.settings.theme_advanced_snips, 'hash'), function(v, k) {
					c.add(k, v, {'class' : 'mce_snip mce_' + v});
				});
			}
...
*/
		...
		_addControls : function(v, tb) {
			...
			each(explode(v), function(n) {
				...
				if (n == 'tablecontrols') {
					each(["row_before","row_after","delete_row","|","col_before","col_after","delete_col","|","split_cells","merge_cells"], function(n) {
